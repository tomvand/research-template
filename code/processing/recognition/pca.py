...
def main():
    ...
    output_fn = os.path.join(output_dir, '{name}.npy'.format(name=os.path.splitext(args.train_data)[0]))
    np.save(output_fn, pca.transform(train_data))

    output_fn = os.path.join(output_dir, '{name}_pca.p'.format(name=os.path.splitext(args.train_data)[0]))
    ...
    for test_fn in args.test_data:
        ...
        output_fn = os.path.join(output_dir, '{name}.npy'.format(name=os.path.splitext(test_fn)[0]))
